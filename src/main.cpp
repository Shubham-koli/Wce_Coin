...
#if defined(NDEBUG)
# error "Wcecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc59abfa99d51f7c3173ae3c1efea861d5d062cb57282f9348b7b14cfa154f9ca");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wcecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wcecoin Signed Message:\n";
...
{
    // Wcecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Wcecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Wcecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Wcecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wcecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wcecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0aaf6a34862e6ff255a8f501c4a45089cdc36234e18e0e2ebf695b1312d3d3a7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cancer ‘vaccine’ eliminates tumors in mice - 90 of 90 mice cured of cancers with lymphoma";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e84e721c9bb629fab7fed4c81a97d48e372efdad3463ba5f6434bf5be4d722527ce11f27ffdda8e5cdb0e5d59d50b9967df2c7a599038ffdf29dcd8b39426dea") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517734704;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086356826;

        if (fTestNet)
        {
            block.nTime    = 1517734676;
            block.nNonce   = 386232172;
        }
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x112f96a2c62cbd54729f943e5277963eb6bfe4a30779f541e84e463be69c444c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Wcecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WcecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WcecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WcecoinMiner(CWallet *pwallet)
{
    printf("WcecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wcecoin-miner");
    ...
    try { loop {
        ...
        printf("Running WcecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WcecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WcecoinMiner, pwallet));
}
